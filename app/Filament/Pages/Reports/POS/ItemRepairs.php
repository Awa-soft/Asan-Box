...
class ItemRepairs extends Page
{
    ...
    public function mount($from,$to,$warehouse,$branch,$type,$item): void{
        ...
        $this->to = $to;
        $this->type = $type;
        $this->data = ItemRepair::when($branches!=[],function ($q)use($branches){
            return $q->where('ownerable_type','App\Models\Logistic\Branch')
                ->whereIn('ownerable_id',$branches);
        })->when($warehouses!=[],function ($q)use($warehouses){
            return $q->where('ownerable_type','App\Models\Logistic\Warehouse')
